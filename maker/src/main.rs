...
fn main() -> anyhow::Result<()>{
    ...
    let sync_status = l2_client.sync_status()?;
    tracing::info!("sync_status = {:?}", sync_status);

    let oracle = Arc::new(CachingOracle::new(1024_000, ORACLE_READER, HINT_WRITER));
    ...
    for n in 0..10 {
        ...
        let claiming_l2_hash = l2_client.get_block_by_number(claiming_l2_number)?.hash;
        let claiming_output_root = l2_client.output_root_at(claiming_l2_number)?;

        let agreed_l2_hash = l2_client.get_block_by_number(sync_status.finalized_l2.number - i - 1)?.hash;
        let agreed_output_root = l2_client.output_root_at(sync_status.finalized_l2.number - i - 1)?;
